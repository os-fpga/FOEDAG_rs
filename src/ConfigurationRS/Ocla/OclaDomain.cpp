...
std::vector<OclaProbe>& OclaDomain::get_probes() {
    return m_probes;
}

std::vector<OclaInstance>& OclaDomain::get_instances() {
    return m_instances;
}

ocla_config OclaDomain::get_config() const { return m_config; }

void OclaDomain::set_config(ocla_config &config) {
    m_config = config;
}

void OclaDomain::add_trigger(oc_trigger_t &trig) {
    m_triggers.push_back(trig);
}

std::vector<oc_trigger_t> OclaDomain::get_triggers() {
    return m_triggers;
}
