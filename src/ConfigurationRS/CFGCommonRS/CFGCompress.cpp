...
void CFG_COMPRESS::analyze_repeat(const uint8_t* input, const size_t input_size,
  ...
  if (length) {
    ...
    if (debug) {
      CFG_POST_DBG(
          "Found repated %ld byte(s) of data for %ld time(s) at index %ld",
          length, repeat, index);
    }
  }
}
...
void CFG_COMPRESS::compress_data(std::vector<uint8_t>& output,
  ...
  if (debug) {
    if (followup_byte == -1) {
      CFG_POST_DBG("%sCompress %ld data of 0x%02X", space.c_str(), length,
                   compress_byte);
    } else {
      CFG_POST_DBG("%sCompress %ld data of 0x%02X - followed by 0x%02X",
...
void CFG_COMPRESS::pack_data(const uint8_t* input, const size_t input_size,
  ...
  CFG_ASSERT(length);
  if (debug) {
    CFG_POST_DBG("Pack %ld byte(s) data at index 0x%016lX", length, index);
  }
    ...
    if (compress_size == 0 && uncompress_size == 0) {
      ...
      if (compress_size == 1) {
        if ((input[index] == 0 || input[index] == 0xFF) &&
          ...
          if (debug) {
            CFG_POST_DBG("Compress 0x%02X followed by %ld data of 0x%02X",
  ...
  while (index < input_size) {
    ...
    if (length) {
      ...
      if (chunk_pattern == CFG_CMP_ZERO || chunk_pattern == CFG_CMP_HIGH ||
        ...
        CFG_ASSERT(index <= input_size);
        if (debug) {
          CFG_POST_DBG("   Pattern: %ld", chunk_pattern);
        }
      }
    } else {
      CFG_ASSERT(repeat == 0);
